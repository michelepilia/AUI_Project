    ...
    public class TestMidiStream : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            if (midiStreamPlayer != null)
            {
                ...
                GUILayout.Label(string.Format("Played:{0} Active:{1} Free: {2} Reused:{3} %",
                    midiStreamPlayer.MPTK_StatVoicePlayed, midiStreamPlayer.MPTK_StatVoiceCountActive, midiStreamPlayer.MPTK_StatVoiceCountFree, Mathf.RoundToInt(midiStreamPlayer.MPTK_StatVoiceRatioReused)), myStyle.TitleLabel3, GUILayout.Width(500));
                GUILayout.EndHorizontal();

                GUILayout.Space(spaceVertival);

                GUILayout.BeginHorizontal(GUILayout.Width(350));
                DelayTimeChange = Slider("Delay note", DelayTimeChange, 0.01f, 10f);
                DeltaDelay = Slider("Delta delay", DeltaDelay, -10f, 10f, true, 80);
                GUILayout.EndHorizontal();
