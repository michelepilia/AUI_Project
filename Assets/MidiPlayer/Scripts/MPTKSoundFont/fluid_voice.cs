    ...
    public class fluid_voice
    {
        ...
        static int[] list_of_weakgenerators_to_initialize =
     {
                ...
                case (int)fluid_gen_type.GEN_VOLENVATTACK:               /* SF2.01 section 8.1.3 # 34 */
                    {
                        ...
                        if (synth.VerboseCalcGen) Debug.Log(string.Format("Calc {0} x={1:0.00} count={2} ms. incr={3}", (fluid_gen_type)igen, x,
                ...
                case (int)fluid_gen_type.GEN_KEYTOVOLENVDECAY:          /* SF2.01 section 8.1.3 # 40 */
                    {
                        ...
                        uint count = synth.MPTK_CorePlayer ?
                            calculate_hold_decay_buffers((int)fluid_gen_type.GEN_VOLENVDECAY, (int)fluid_gen_type.GEN_KEYTOVOLENVDECAY, true) : /* 1 for decay */
                            (uint)(Nano100ToMilli * calculate_hold_decay_ms((int)fluid_gen_type.GEN_VOLENVDECAY, (int)fluid_gen_type.GEN_KEYTOVOLENVDECAY, true)); /* 1 for decay */
                        ...
                        if (synth.VerboseCalcGen) Debug.Log(string.Format("Calc {0} y={1:0.00} count={2} incr={3}", (fluid_gen_type)igen, y,
                ...
                case (int)fluid_gen_type.GEN_VOLENVRELEASE:             /* SF2.01 section 8.1.3 # 38 */
                    {
                        ...
                        if (synth.VerboseCalcGen) Debug.Log(string.Format("Calc {0} x={1:0.00} count={2} ms. incr={3} fluid_tc2sec_release(x)={4}",
                ...
                case (int)fluid_gen_type.GEN_MODENVRELEASE:             /* SF2.01 section 8.1.3 # 30 */
                    {
                        ...
                        if (synth.VerboseCalcGen) Debug.Log(string.Format("Calc {0} x={1:0.00} count={2} ms. incr={3} tc2msec_release(x)={4}",
                            (fluid_gen_type)igen, x,
                    ...
                    float[] dsp_reverb_buf, float[] dsp_chorus_buf)
        {
            ...
            //Debug.LogFormat("t:{0} calc --> coeff:{1} * volenv_val:{2} + incr:{3} --> x:{4} section:{5}", TimeFromStart, env_data.coeff, volenv_val, env_data.incr, x, volenv_section);
        ...
        public int fluid_voice_kill_excl()
        {
            ...
            if (synth.VerboseEnvVolume) DebugVolEnv("kill exclusif");
        ...
        */
        ...
        {
            if (!weakDevice)
                Debug.LogFormat("VolEnv - [{0:4}] {1} TimeFromStart:{2:0.000} Delta:{3:0.000} section:{4} volenv_val:{5:0.000} volenv_count:{6} incr:{7:0.0000}",
                   IdVoice, info, TimeFromStart / Nano100ToMilli, DeltaTimeWrite / Nano100ToMilli,
        ...
        public void DebugLFO(string info)
        {
            Debug.LogFormat("[{0:4}] {1} TimeFromStart:{2:00000.000} Delta:{3:0.000} modlfo_delay:{4} modlfo_incr:{5:0.000} modlfo_val:{6:0.000} modlfo_to_vol:{7:0.000}",
               IdVoice, info, TimeFromStart / Nano100ToMilli, DeltaTimeWrite / Nano100ToMilli, modlfo_delay, modlfo_incr, modlfo_val, modlfo_to_vol);
        }
        public void DebugVib(string info)
        {
            Debug.LogFormat("[{0:4}] {1} TimeFromStart:{2:00000.000} Delta:{3:0.000} viblfo_delay:{4} viblfo_incr:{5:0.000} viblfo_val:{6:0.000} viblfo_to_pitch:{7:0.000} -. pitch mod:{8:0.000}",
               IdVoice, info, TimeFromStart / Nano100ToMilli, DeltaTimeWrite / Nano100ToMilli, viblfo_delay, viblfo_incr, viblfo_val, viblfo_to_pitch, (float)(1d + viblfo_val * viblfo_to_pitch / 1000d));
        }
    }
}
